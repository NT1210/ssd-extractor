            ...
            if(check.doubled){
                for(let i=0; i<100; i++){
                    if(sheet_katsu.row(check.row+i).cell(producer_column).value() === katsu.producer &&
                        ...
                        if( sheet_katsu.row(check.row+i).cell(f_ground_column).value()){
                ...
                if(!done){
                    for(let i=7; i<101; i++){
                        if(sheet_katsu.row(i).cell(vessel_column).value() === undefined ){
                            ...
                            // below is about fishing ground
                            if( sheet_katsu.row(i).cell(f_ground_column).value()){
                            ...
                            sheet_katsu.row(i).cell(type_column).value("タラバ")
...
function funcBoil(sheet_boil, target_date_column, matched_lists, workbook2, dateToBeInserted, desktopDir){
    ...
    const order = ["5L", "4L", "3L", "2L", "L", "M", "H", "М", "Н", "цел", "печень", "абд"]
    ...
    sortedTypeListObj.boil.forEach(boil => {
        ...
        try{
            if(check.doubled){
                ...
                let target_size_list = ["5L", "4L", "3L", "2L", "L", "M", "H", "М", "Н", "цел", "печень", "абд"]
                ...
                if(size_detected === "печень") size_detected = "カニミソ"
                ...
                if( sheet_boil.row(check.row).cell(producer_column).value() === boil.producer && sheet_boil.row(check.row).cell(size_column).value() === size_detected){
                    ...
                    for(let j=1; j<9; j++){
                        if(sheet_boil.row(check.row+j).cell(vessel_column).value() === undefined){
                            ...
                            sheet_boil.row(check.row+j).cell(type_column).value("タラバ")
                            ...
                            if(size_detected === "цел") sheet_boil.row(check.row+j).cell(size_column).value("ラウンド")
                            if(size_detected === "печень") sheet_boil.row(check.row+j).cell(size_column).value("カニミソ")
                            if(size_detected === "абд") sheet_boil.row(check.row+j).cell(size_column).value("腹肉")
                ...
                if(lowest_bottom_row <= 0){
                    ...
                    sheet_boil.row(7).cell(type_column).value("タラバ")
                    ...
                    sheet_boil.row(row_for_new_vessel).cell(type_column).value("タラバ")
...
function funcNama(sheet_nama, target_date_column, matched_lists, workbook2, dateToBeInserted, desktopDir){
    ...
    const order = ["5L", "4L", "3L", "2L", "L", "M", "H","М", "Н"]
    ...
    sortedTypeListObj.nama.forEach(nama => {
        ...
        try{
            if( check.doubled){
                ...
                // let target_size_list = ["5L", "4L", "3L", "2L", "L", "M", "H", "цел", "печень", "абд"]
                let target_size_list = ["5L", "4L", "3L", "2L", "L", "M", "H","М", "Н"]
                ...
                if( sheet_nama.row(check.row).cell(producer_column).value() === nama.producer && sheet_nama.row(check.row).cell(size_column).value() === size_detected){
                    ...
                    for(let j=1; j<9; j++){
                        if(sheet_nama.row(check.row+j).cell(vessel_column).value() === undefined){
                            ...
                            sheet_nama.row(check.row+j).cell(type_column).value("タラバ")
                            ...
                            // convert from ru to jp if product name is one of the three products below
                            // if(size_detected === "цел") sheet_nama.row(check.row+j).cell(size_column).value("ラウンド")
                            // if(size_detected === "печень") sheet_nama.row(check.row+j).cell(size_column).value("カニミソ")
                            // if(size_detected === "абд") sheet_nama.row(check.row+j).cell(size_column).value("腹肉")
                ...
                if(lowest_bottom_row <= 0){
                    ...
                    sheet_nama.row(7).cell(type_column).value("タラバ")
                    ...
                    sheet_nama.row(row_for_new_vessel).cell(type_column).value("タラバ")
