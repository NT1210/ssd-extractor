...
const CrabOPPreExecute = ({setPage, setExecCheck, setSsdCheck, setDouseiCheck, setDBCheck}) => {
  ...
  const extract = async (arg) => {
    setExecCheck(true)
    setLoading(true)
    let data = await window.myAPI.extractCrabOP(arg)
    setLoading(false)
    setTimeout(()=> {
        toast({
            title: 'お知らせ',
...
export default CrabOPPreExecute